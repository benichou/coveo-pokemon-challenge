...
- **Trunk-based development.** `main` is the single shared trunk; all work lands on it directly (or via very short-lived branches that merge within hours). No long-lived feature branches, no GitFlow. Every push to `main` is gated by `pr-checks.yml` (pre-commit + tests + build + TruffleHog) — same gates whether the path is a direct push or a PR, so quality stays high without long-lived parallel branches accumulating drift. PRs are optional, not required (one-human personal repo). The four legacy `feature/*` branches in the remote are abandoned and exist only for historical traceability.
